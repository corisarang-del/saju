...
## 2026-07-24 하나 궁합상담 수정 커밋/푸시 완료
- 하나 궁합 상대방 정보 반영 수정분은 커밋 `06d7fde 하나 궁합 상대정보 반영`으로 저장했다.
- 원격 `fork/main`에 푸시 완료했다.
- 최종 검증은 전체 vitest 57개 파일/263개 테스트, tsc, eslint, git diff check, Next production build 통과다.
- 기본 샌드박스의 Next build는 Turbopack 포트 바인딩 제한으로 실패했지만, 승인 경로에서 같은 빌드가 통과했다.
- 커밋 직후 작업트리는 깨끗했다.
- 최신 기록 문서: `docs/개발일지/하나-궁합상담-상대방정보-반영-커밋푸시-기록-20260724.md`.

## 2026-07-24 배포웹사이트 프리미엄마감 최종재리뷰 Findings
- `design-taste-frontend`는 제외하고 `high-end-visual-design`, `minimalist-ui`, `redesign-existing-projects` 기준으로 production을 재리뷰했다.
- 최신 production 기준 새 P0/P1/P2 디자인 이슈는 없다.
- 해결됨: PC 1366x768 `/ko`에서 첫 가시 카드 4개(`현우`, `하나`, `민준`, `인영`)의 `대화하기` CTA가 viewport 안에 완전히 들어온다. CTA bottom은 `586px`, viewport height는 `768px`이다.
- 해결 유지: PC 1280x720 `/ko`에서 CTA bottom은 `586px`, viewport height는 `720px`이고, 7초 후에도 `scrollLeft=0`, 첫 가시 카드 `현우`가 유지된다.
- 해결 유지: 모바일 390x844 `/ko`에서 `현우` 이름은 잘리지 않고 CTA bottom은 `736px`으로 하단 탭 위에 안정적으로 보인다.
- 해결 유지: 모바일 390x844 `/ko/reading`에서 하단 `다음` CTA는 `x=20`, `y=752`, `w=350`, `h=60`, `bottom=812`로 viewport 안에 완전히 들어온다.
- `/ko`, `/ko/reading` console error/warning 0건, 가로 오버플로우 없음이다.
- 남은 항목은 P3 관찰 수준이다. 우측 edge preview 카드는 현재 캐러셀 affordance로 읽히며, 보라색 accent hierarchy도 CTA 중심으로 충분히 정리됐다.
- 개발자 전달 문서: `docs/pm/배포웹사이트-프리미엄마감-최종재리뷰-20260724.md`.

## 2026-07-24 무료베타 유료전환 다음 PM 우선순위
- 총괄 PM 판정상 무료 베타 사이트는 제대로 설계되고 구현된 상태로 본다.
- 유료 코칭형 사주앱 방향은 맞지만, 결제 운영 안정성, 상담 품질 반복성, 구독 가치 증명이 아직 남아 있다.
- Paddle 제외 무료 베타 운영은 유지하면서 실제 상담 품질과 퍼널 데이터를 모은다.
- 다음 개발 우선순위는 첫 상담 응답 속도/실패 복구 UX 안정화, 5대 고민별 첫 답변 품질 캡처, 캐릭터별 차이 검수, 스타터 `10별 2,900원` 첫 결제 1회 한정 정책, 월구독 `50별` 사용 예시 강화, `release:gate:payments:live`와 signed webhook QA 결제 gate 고정, 무료 베타 지표 기반 유료 전환 검증이다.
- 5대 고민 QA 대상은 `썸/재회`, `이직/퇴사`, `돈 모으기`, `번아웃`, `친구/가족관계`다.
- 월구독 가치는 `매일 1회 상담 30별 + 월간 리포트 3별 + 후속 질문 17회`처럼 사용 장면으로 설명한다.
- 결제 버튼은 Paddle production env, 실제 price id, signed webhook live QA가 닫히기 전까지 열지 않는다.
- 개발자 전달 문서: `docs/pm/무료베타-유료전환-다음PM우선순위-개발자전달-20260724.md`.

## 2026-07-24 production 캐릭터별 상담 QA 잔여이슈
- 최신 production `https://monthlysaju.vercel.app/` 기준 기본 상담 기능은 정상이다. `/ko` 렌더링, 무인증 `/api/saju/chat` 401 JSON, analytics track 200, 비로그인 분석 시작 후 Google 로그인 CTA, 무료/전체 live API QA가 통과했다.
- 캐릭터별 상담 품질은 전체 합격이 아니다. 인영(`haeun`)이 2회 연속 쉬운 말/한자 과다/오늘 행동 기준을 실패했다.
- 인영 실패 preview에는 `병오(丙午)`, `경(庚)`, `비견(比肩)`처럼 한자와 전문용어가 초반에 몰렸다. `haeun` 전용 initial prompt와 품질 게이트 회귀 테스트가 필요하다.
- 도윤(`doyun`)은 1차 전체 캐릭터 QA에서 50479ms 뒤 503으로 실패했고, 재시도에서는 7839ms/200으로 통과했다. 간헐 provider timeout 또는 fallback 부재를 확인해야 한다.
- 현우(`charon_m`) 48760ms, 서준(`seojun`) 39066ms처럼 통과 응답도 오래 걸리는 케이스가 있다. 첫 의미 있는 응답 15초 안쪽, 전체 완료 30초 안쪽을 목표로 본다.
- 하나/민준/서준 일부 응답은 시작 문장이 비슷해 캐릭터 차별감이 약하다. 유료 전환 기준에서는 캐릭터별 opening pattern을 분리해야 한다.
- 개발자 전달 문서: `docs/pm/production-캐릭터별-상담QA-잔여이슈-개발자전달-20260724.md`.

## 2026-07-24 무료베타 유료전환 전 계측/UX 반영
- `docs/pm/무료베타-유료전환-다음PM우선순위-개발자전달-20260724.md` 기준으로 결제는 닫아둔 채 무료 베타 퍼널 계측과 첫 상담 대기 UX를 보강했다.
- 신규 퍼널 이벤트는 랜딩, 입력 페이지, 생년월일 완료, 성별 완료, 캐릭터 선택, 로그인 시작/성공, 첫 상담 시작/성공/실패, 후속 질문, 무료 할당량 소진, 가격 패널/코인샵/결제 비활성 안내 노출이다.
- 첫 상담 로딩 문구는 0초 `사주 흐름을 정리하고 있어`, 10초 `고민에 맞는 상담 방향을 잡고 있어`, 30초 `답변이 조금 길어지고 있어. 완료되면 바로 보여줄게`로 단계화했다.
- 월구독 50별 가치는 `매일 1회 상담 30별 + 월간 리포트 3별 + 후속 질문 17회` 예시로 랜딩, FAQ, 코인샵에 노출한다.
- 스타터 첫 결제 1회 제한은 서버/DB/webhook/admin 정책이 필요한 후속 작업이라 이번 구현에서는 결제를 열지 않았다.

## 2026-07-24 production 캐릭터별 상담 QA 잔여이슈 반영
- 인영(`haeun`) 첫 상담은 이제 일반 종합 사주 질문이 아니라 `2026년`, `이번 달`, `이번 주`, `오늘 할 행동` 중심의 시기 상담으로 시작한다.
- 인영 fallback도 시기형 2문단으로 분리해 한자/십신 없이 사주 근거, 오늘 행동, 질문 마무리를 지킨다.
- 첫 상담 품질 게이트는 한자 병기뿐 아니라 `병오`, `일간`, `비견`, `세운`, `대운`, `월운`, `자미성` 같은 한글 전문용어도 저장 전 차단한다.
- 도윤(`doyun`) 같은 간헐 provider 지연은 첫 상담 생성 시도별 25초 timeout, provider 오류 로그, 안전 fallback으로 방어한다.
- 배포 후 캐릭터별 live QA는 7개 캐릭터 각각 3회 이상 반복해 status, 저장된 assistant, 별 차감, 첫 200자 쉬운 말, 2문단, 오늘 행동, 질문 마무리를 다시 확인해야 한다.
