...
- 무료 베타 유료전환 전 보강에서는 월구독 50별 사용 예시만 노출하고 결제 활성화는 하지 않는다.
...
## DEC-053: 유료 전환 전 무료 베타 데이터와 결제 live gate를 먼저 닫는다

**날짜**: 2026-07-24
**상태**: 승인됨

**결정**:
무료 베타 운영은 유지하되, 유료 결제 버튼은 Paddle `release:gate:payments:live`와 signed webhook QA가 통과하기 전까지 열지 않는다. 유료 전환 작업은 실제 상담 품질 데이터, 첫 상담 실패 복구, 5대 고민별 답변 품질, 스타터 첫 결제 1회 한정, 월구독 50별 사용 예시를 먼저 닫은 뒤 진행한다.

**근거**:
총괄 PM 리뷰에서 무료 베타 사이트는 제대로 설계되고 구현된 상태로 판단했다. 하지만 유료 코칭형 사주앱은 결제 운영 안정성, 상담 품질 반복성, 구독 가치 증명이 아직 남아 있다. 결제 실패나 별 미지급은 사주/상담 카테고리에서 신뢰 회복이 어렵기 때문에 payment gate가 닫히기 전 결제 오픈은 금지한다.

**우선순위**:
- Paddle 제외 무료 베타 운영 데이터를 모은다.
- 첫 상담 응답 속도와 실패 복구 UX를 안정화한다.
- `썸/재회`, `이직/퇴사`, `돈 모으기`, `번아웃`, `친구/가족관계` 첫 답변을 캡처해 캐릭터별 차이를 검수한다.
- `10별 2,900원` 스타터는 첫 결제 1회 한정 정책으로 확정한다.
- 월구독 `50별`은 `매일 1회 상담 30별 + 월간 리포트 3별 + 후속 질문 17회`처럼 사용 예시로 설명한다.
- 결제 재오픈 전 `release:gate:payments:live`와 signed webhook QA를 필수 gate로 둔다.

**검증 기준**:
- 무료 베타 지표에서 가입 전환, 첫 상담 완료율, 후속 질문률, 무료 3회 소진율, D1/D7 재방문율을 볼 수 있다.
- 첫 상담 실패/빈 응답/부분 응답에서는 assistant 저장, 별 차감, 무료 사용량 증가가 발생하지 않는다.
- 스타터 구매 이력이 있는 사용자는 `10별 2,900원`을 반복 구매할 수 없다.
- 결제 UI 활성화는 Paddle production env, 실제 price id, signed webhook 성공/중복/실패/환불/구독 QA 통과 이후에만 가능하다.

**구현 메모**:
- 2026-07-24 무료 베타 퍼널 이벤트 타입과 클라이언트 계측 helper를 추가했다.
- 랜딩/입력/캐릭터 선택/로그인 시작/로그인 성공/채팅/코인샵 노출 계측을 붙였다.
- 첫 상담 대기 문구를 0초, 10초, 30초 기준으로 단계화했다.
- 결제 버튼은 계속 feature flag로 닫아둔다.

---

## DEC-054: 첫 상담은 어려운 사주 용어보다 쉬운 생활 언어를 우선한다

**날짜**: 2026-07-24
**상태**: 승인됨

**결정**:
첫 상담 저장 전 품질 게이트는 한자 병기뿐 아니라 `병오`, `일간`, `비견`, `세운`, `대운`, `월운`, `명궁`, `자미성` 같은 한글 사주·자미두수 전문용어도 차단한다. 첫 상담은 정확히 2문단, 사주 근거, 오늘 할 행동, 질문 마무리, 쉬운 한국어 기준을 우선한다.

**근거**:
production 캐릭터별 QA에서 인영(`haeun`)이 분야는 맞았지만 초반 전문용어와 한자가 몰려 20대 여성 고객 기준 쉬운 말/오늘 행동 기준을 실패했다. 첫 상담은 유료 전환 전 신뢰를 결정하는 구간이라 전문 풀이보다 이해 가능한 말이 먼저다.

**구현 메모**:
- `haeun` 첫 상담 prompt와 fallback을 시기형으로 분리했다.
- `chat-completion-guard`의 첫 상담 차단어에 한글 전문용어를 추가했다.
- 인영 원본 프롬프트에는 첫 상담에서 한자 병기와 어려운 사주 용어를 앞에 꺼내지 말라는 규칙을 넣었다.

---

## DEC-055: 첫 상담 provider 지연은 timeout 후 안전 fallback으로 처리한다

**날짜**: 2026-07-24
**상태**: 승인됨

**결정**:
첫 상담 사전 생성은 시도별 25초 timeout을 둔다. provider 오류가 빠르게 발생하면 다음 시도를 허용하고, timeout성 장기 지연이면 더 기다리지 않고 안전 fallback 경로로 전환한다.

**근거**:
production QA에서 도윤(`doyun`)이 약 50초 뒤 503으로 실패했다. 사용자가 50초를 기다린 뒤 빈 실패를 받는 경험은 무료 베타와 유료 전환 모두에서 신뢰를 크게 떨어뜨린다.

**검증 기준**:
- 첫 상담 생성 로그에 전체 `durationMs`와 `attemptDurationsMs`가 남는다.
- provider 오류는 `Initial analysis attempt provider error`로 기록된다.
- 품질 실패는 `Initial analysis attempt failed quality gate`로 기록된다.
- fallback도 저장 전 품질 게이트를 통과해야 한다.
