...
- 사주 용어는 생활 언어로 풀어줘.
- 답변은 1~3문단으로 모바일에서 부담 없게 써.
- 압박, 공포 마케팅, 과장된 예언, 하대 표현을 쓰지 마.
- 끝에는 바로 이어서 답할 수 있는 질문을 1개만 남겨.
...
function evaluateAnswer(text, testCase) {
  ...
  const hasFollowUpQuestion = /[?？]\s*$/.test(text.trim()) || text.includes("어떤 쪽");
...
function renderReport(results) {
  ...
  const lines = [
    ...
    ...results.map(({ testCase, evaluation }) =>
      `| ${testCase.label} | ${evaluation.reflectedConcern ? "통과" : "확인필요"} | ${!evaluation.hasBlockedPhrase ? "통과" : "확인필요"} | ${evaluation.mobileFriendlyLength ? "통과" : "확인필요"} | ${evaluation.hasFollowUpQuestion ? "통과" : "확인필요"} |`,
    ),
    "",
  ];
