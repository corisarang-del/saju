...
function buildSafeInitialAnalysisFallback(params: {
...
export async function POST(req: Request) {
  ...
  const chatGenerationLockKey = user.id;
  ...
  const releaseCurrentChatGenerationLock = () => {
  ...
  try {
    ...
    releaseCurrentChatGenerationLock();
    console.error("[saju/chat] failed to reserve chat star", {
      readingId,
      userId: user.id,
      error: serializeChatProviderError(error),
